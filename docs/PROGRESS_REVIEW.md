...
| Task | Plan | Implemented | Gaps |
|:-----|:-----|:-----------|:-----|
| T15: Docker image tagging + GHCR | CI push versioned images | ✅ `.github/workflows/docker-publish.yml` builds and pushes each service Dockerfile to `ghcr.io` with `:sha` and `:latest` on pushes to `main` | Requires GitHub `packages` write permission (workflow included) |
| T16: Compose profiles | full, water_only, security | ✅ `docker-compose.profile-water-only.yml`, **`docker-compose.profile-security.yml`**, **`docker-compose.profile-monitoring.yml`** (overlay stubs + documentation) | Profiles are minimal `services: {}` overlays — expand with real service lists per environment |
